...
//     }());
//
//     String id = '${objectRuntimeType(child, '<dont know>')} ${shortHash(child)}';
//
...
//       bool needsCompositing, Offset offset, Rect bounds, RRect clipRRect, PaintingContextCallback painter,
...
//       bool needsCompositing, Offset offset, Rect bounds, Path clipPath, PaintingContextCallback painter,
...
//   /// ancestor render objects that this render object will include a composited
//   /// layer, which, for example, causes them to use composited clips.
//   ColorFilterLayer pushColorFilter(Offset offset, ColorFilter colorFilter, PaintingContextCallback painter,
...
//       bool needsCompositing, Offset offset, Matrix4 transform, PaintingContextCallback painter,
...
//     if (_relayoutBoundary == null) return true; // we don't know where our relayout boundary is yet
//     RenderObject node = this;
...
//       if (owner != null) {
//         assert(() {
//           if (debugPrintMarkNeedsLayoutStacks) debugPrintStack(label: 'markNeedsLayout() called for $this');
//           return true;
//         }());
...
//           final String? problemFunction = (targetFrameMatch != null && targetFrameMatch.groupCount > 0)
//               ? targetFrameMatch.group(1)
//               : stack[targetFrame].trim();
//           // TODO(jacobr): this case is similar to displaying a single stack frame.
//           yield ErrorDescription("These invalid constraints were provided to $runtimeType's layout() "
...
//
//   void markNeedsPaint() {
//     String id = '${objectRuntimeType(this, '<dont know>')} ${shortHash(this)}';
//
...
//       assert(() {
//         if (debugPrintMarkNeedsPaintStacks) debugPrintStack(label: 'markNeedsPaint() called for $this');
//         return true;
//       }());
...
//         if (node._layer == null) break; // looks like the subtree here has never been painted. let it handle itself.
//         if (node._layer!.attached)
...
//           describeForError('The following RenderObject was already being painted when it was '
//               'painted again'),
//           ErrorDescription('Since this typically indicates an infinite recursion, it is '
//               'disallowed.'),
//         ]);
//       }
//       return true;
...
//               ErrorDescription('A RenderObject with children must implement visitChildren and '
...
//         }
//         throw FlutterError.fromParts(<DiagnosticsNode>[
//           ErrorSummary('Tried to paint a RenderObject before its compositing bits were '
//               'updated.'),
//           describeForError(
//             'The following RenderObject was marked as having dirty compositing '
//             'bits at the time that it was painted',
//           ),
//           ErrorDescription('A RenderObject that still has dirty compositing bits cannot be '
...
//     }
//     if (debugRepaintRainbowEnabled) renderObjectHelper.debugPaintPaintInfo(context, offset);
//     assert(() {
...
//     }
//
//     for (final _InterestingSemanticsFragment fragment in toBeMarkedExplicit) fragment.markAsExplicit();
//
//     _needsSemanticsUpdate = false;
//
...
//         tooltip: _debugCanParentUseSize == true ? 'can use size' : null, missingIfNull: true));
...
//         throw FlutterError.fromParts(<DiagnosticsNode>[
//           ErrorSummary('A $runtimeType expected a child of type $ChildType but received a '
...
//           ErrorSummary('A $runtimeType expected a child of type $ChildType but received a '
//               'child of type ${child.runtimeType}.'),
//           ErrorDescription('RenderObjects expect specific types of children because they '
//               'coordinate with their children during layout and paint. For '
//               'example, a RenderSliver cannot be the child of a RenderBox because '
//               'a RenderSliver does not understand the RenderBox layout protocol.'),
//           ErrorSpacer(),
...
//         : null;
//
//     if (!_mergeIntoParent && (geometry?.dropFromTree == true)) return; // Drop the node, it's not going to be visible.
//
...
class PaintingContext extends ClipContext {
  ...
  ///  * [repaintCompositedChild], for repainting a composited child without
  ///    instrumentation.
  static void debugInstrumentRepaintCompositedChild(RenderObject child, {
    bool debugAlsoPaintedParent = false,
    required PaintingContext customContext,
  }) {
    assert(() {
  ...
  void paintChild(RenderObject child, Offset offset) {
    if (child.isPainting)
      print('PPPPPPPPAAAAAintinh');
    assert(() {
      debugOnProfilePaint?.call(child);
      return true;
    }());
    child.isPainting = true;
    String id = '${objectRuntimeType(child, '<dont know>')} ${shortHash(child)}';
  ...
  void _compositeChild(RenderObject child, Offset offset) {
    ...
    assert(_canvas == null || _canvas!.getSaveCount() == 1);
    if (child.isPainting) {
      print('comp PPPPPPPPAAAAAintinh ${child.id} ${child._needsPaint}');
      child.renderObjectHelper.logEvent(PaintEvent(eventType: PaintEventType.paintBoundary, id: child.id));
    }
    // Create a layer for our child, and paint the child into it.
    child.isPainting = true;
    if (child._needsPaint) {
  ...
  String toString() => '${objectRuntimeType(
      this, 'PaintingContext')}#$hashCode(layer: $_containerLayer, canvas bounds: $estimatedBounds)';
}
...
abstract class RenderObject extends AbstractNode with DiagnosticableTreeMixin implements HitTestTarget {
  ...
  String get id {
    _id ??= '${objectRuntimeType(this, '<dont know>')} ${shortHash(this)}';
    return _id!;
  }
  ...
  /// to the tree. If new [SemanticsNode]s are instantiated in this method
  /// they must be disposed in [clearSemantics].
  void assembleSemanticsNode(SemanticsNode node,
      SemanticsConfiguration config,
      Iterable<SemanticsNode> children,) {
    assert(node == _semantics);
    // TODO(a14n): remove the following cast by updating type of parameter in either updateWith or assembleSemanticsNode
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(FlagProperty('needsCompositing', value: _needsCompositing, ifTrue: 'needs compositing'));
    properties.add(
        DiagnosticsProperty<Object?>('creator', debugCreator, defaultValue: null, level: DiagnosticLevel.debug));
    properties.add(DiagnosticsProperty<ParentData>(
        'parentData', parentData, tooltip: (_debugCanParentUseSize ?? false) ? 'can use size' : null,
        missingIfNull: true));
    properties.add(DiagnosticsProperty<Constraints>('constraints', _constraints, missingIfNull: true));
    // don't access it via the "layer" getter since that's only valid when we don't need paint
...
mixin ContainerRenderObjectMixin<ChildType extends RenderObject, ParentDataType extends ContainerParentDataMixin<
    ChildType>> on RenderObject {
  bool _debugUltimatePreviousSiblingOf(ChildType child, { ChildType? equals }) {
    ParentDataType childParentData = child.parentData! as ParentDataType;
    while (childParentData.previousSibling != null) {
      assert(childParentData.previousSibling != child);
  ...
  void _insertIntoChildList(ChildType child, { ChildType? after }) {
    ...
    if (after == null) {
      ...
      if (afterParentData.nextSibling == null) {
        ...
        childParentData.previousSibling = after;
        // set up links from siblings to child
        final ParentDataType childPreviousSiblingParentData = childParentData.previousSibling!
  ...
  void insert(ChildType child, { ChildType? after }) {
    assert(child != this, 'A RenderObject cannot be inserted into itself.');
    assert(after !=
        this, 'A RenderObject cannot simultaneously be both the parent and the sibling of another RenderObject.');
    assert(child != after, 'A RenderObject cannot be inserted after itself.');
    assert(child != _firstChild);
    assert(child != _lastChild);
    adoptChild(child);
...
class _RootSemanticsFragment extends _InterestingSemanticsFragment {
  ...
  @override
  void compileChildren(
      { Rect? parentSemanticsClipRect, Rect? parentPaintClipRect, required double elevationAdjustment, required List<
...
class _SwitchableSemanticsFragment extends _InterestingSemanticsFragment {
  ...
  void compileChildren(
    ...
    final _SemanticsGeometry? geometry = _needsGeometryUpdate
        ? _SemanticsGeometry(parentSemanticsClipRect: parentSemanticsClipRect,
        parentPaintClipRect: parentPaintClipRect,
        ancestors: _ancestorChain)
        : null;
...
class _SemanticsGeometry {
  ...
  //
  // It will mutate both transform and clipRectTransform.
  static void _applyIntermediatePaintTransforms(RenderObject ancestor,
      RenderObject child,
      Matrix4 transform,
      Matrix4 clipRectTransform,) {
    assert(ancestor != null);
...
abstract class RenderObject extends AbstractNode with DiagnosticableTreeMixin implements HitTestTarget {
  ...
  void _paintWithContext(PaintingContext context, Offset offset) {
    ...
    try {
      if (isPainting) {
        // return;
        // print('pwc PPPPPPPPAAAAAintinh $id');
      }
      isPainting = true;
